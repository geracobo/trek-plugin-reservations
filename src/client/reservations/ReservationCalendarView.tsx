...
export function ReservationCalendarView({ reservations, trip }: { reservations: Reservation[]; trip: Trip | null }) {
  ...
  const reservationsByDate = useMemo(() => reservations.reduce<Record<string, Reservation[]>>((byDate, reservation) => {
  ...
  return (
    <section className="trek-card overflow-hidden p-0 max-[720px]:overflow-x-auto">
      <header className="flex items-center justify-center gap-2.5 border-b border-edge-faint px-3 py-2.5 max-[720px]:sticky max-[720px]:left-0 max-[720px]:min-w-[700px]">
        <button type="button" className="trek-btn trek-btn--ghost reservation-calendar-nav min-h-[30px] px-2 py-1 text-xs" onClick={() => changeMonth(-1)} aria-label="Previous month">
          <ChevronLeft className="shrink-0" size={16} />
          <span>Previous</span>
        </button>
        <h2 className="m-0 min-w-[180px] text-center text-sm font-semibold text-content">{month.toLocaleDateString(undefined, { month: 'long', year: 'numeric', timeZone: 'UTC' })}</h2>
        {startMonth ? (
          <button type="button" className="trek-btn trek-btn--ghost reservation-calendar-nav min-h-[30px] px-2 py-1 text-xs" onClick={() => setMonth(startMonth)} title="Go to trip start month">
            <CalendarDays size={14} />
            <span>Trip start</span>
          </button>
        ) : null}
        <button type="button" className="trek-btn trek-btn--ghost reservation-calendar-nav min-h-[30px] px-2 py-1 text-xs" onClick={() => changeMonth(1)} aria-label="Next month">
          <span>Next</span>
      ...
      <div className="grid grid-cols-7 max-[720px]:min-w-[700px]">
        {WEEKDAYS.map((weekday) => <div className="border-b border-edge-faint bg-surface-secondary p-2 text-center text-[10px] font-extrabold uppercase text-content-faint" key={weekday}>{weekday}</div>)}
        {days.map((date) => {
          const key = dateKey(date)
          const entries = reservationsByDate[key] ?? []
          const outsideMonth = date.getUTCMonth() !== month.getUTCMonth()
          return (
            <div className={`min-h-28 min-w-0 border-r border-b border-edge-faint bg-surface p-[7px] [&:nth-child(7n)]:border-r-0 ${outsideMonth ? 'bg-surface-secondary opacity-55' : ''}`} key={key}>
              ...
              <div className="flex flex-col gap-1">
                {entries.map((reservation) => {
                  ...
                  return (
                    <div className="flex min-w-0 items-center gap-1 rounded-[5px] border-l-2 border-l-[var(--reservation-color)] bg-surface-muted px-[5px] py-1 text-[10px] text-content-secondary" key={reservation.id} style={{ '--reservation-color': type.color } as React.CSSProperties} title={reservationTitle(reservation)}>
