  ...
  routes: [
    {
      ...
      async handler(req, ctx) {
        ...
        try {
          ...
          if (unavailable.length > 0) ctx.log.warn(`reservation form data unavailable for trip ${tripId}: ${unavailable.join('; ')}`);

          const places = placesResult.status === 'fulfilled' && Array.isArray(placesResult.value) ? placesResult.value : [];
          const days = daysResult.status === 'fulfilled' && Array.isArray(daysResult.value) ? daysResult.value : [];
          const accommodations = accommodationsResult.status === 'fulfilled' && Array.isArray(accommodationsResult.value) ? accommodationsResult.value : [];
